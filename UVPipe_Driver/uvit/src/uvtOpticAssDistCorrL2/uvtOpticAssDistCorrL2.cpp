/* 
 * File:   uvtOpticAssDistCorrL2.cpp
...
uvtOpticalDestortion_l2::uvtOpticalDestortion_l2 ()
{
...
//    delete[] x_Distortion ;
//    delete[] y_Distortion ;
//    delete[] caldb_xdist_final , caldb_ydist_final ;

}

//parameter File reading

int uvtOpticalDestortion_l2::read (int argc , char** argv)
{
    int status = 0 ;
...
int uvtOpticalDestortion_l2::read (char *inputdatadir , char * caldbDir , char *outdir , int clobber , int history)
{
...
void uvtOpticalDestortion_l2::display ()
{
...
int uvtOpticalDestortion_l2::uvtOpticaDistoProcess ()
{
    ...
    string tempname = caldb_handler.getOpticalDetectorFile (datainfo.getDetector () , caldbDir , datainfo.getFilter ()) ;
    if (tempname == "")
    {
    ...
    else if (datainfo.getModeFlag () == PC)
    {
        ...
        if (opticalDestrotionPC ()) return (EXIT_FAILURE) ;
...
int uvtOpticalDestortion_l2::opticalDistortionIM ()
{

    LOG(INFO) << endl << " Correction of Optical Distortion  for IM mode" ;
    ...
    float frame_data_exp_temp[xsize * ysize] ;
    ...
    for (int i = 0 ; i < nframes ; i++)
    {
        ...
        {
            frame_data[i] = 0.0f ;
            frame_data_temp[i] = 0.0f ;
            frame_data_exp[i] = 0.0f ;
            frame_data_exp_temp[i] = 0.0f ;
        }
       sprintf (errstr , "Error at iteration number %d" , i) ;
        ...
        fits_read_pix (fptr , TFLOAT , fpixel , xsize*ysize , NULL , frame_data , NULL , &status) ;//reading  the Signal Frame
        ...
        for (int i=0;i<xsize*ysize;i++)
        {
            ...
            frame_data_exp_temp[i]=INVALID_PIX_VALUE;
...
//            rown = rown + caldb_ydist_final[rown * caldbfinalsize + coln] ;
//            coln = coln + caldb_xdist_final[rown * caldbfinalsize + coln] ;
...
frame_data_temp[(int) round (rown * xsize + coln)] =INVALID_PIX_VALUE;
            if (round (rown * xsize + coln) < xsize * ysize)
            {
        ...
        sprintf (outfile , "%s/%s/%s_t%.4f_f%d_sig_od.fits" , moduleoutdir , sigframedir , nameprefix , frametime , frameno) ;
        ...
        printError (status , "***Error in creating the image in output frame***",outfile) ;
        fits_write_pix (fout , TFLOAT , fpixel , xsize*ysize , frame_data_temp , &status) ;//writinr the corrected Signal frame to the output frame
        ...
        printError (status , "***Error in closing the output Signal frame***",outfile) ;
        fits_close_file (fptr , &status) ;
        printError (status , "***Error in closing the input Signal frame***",outfile) ;

        sprintf (infile , "%s/%s/%s" , inputdatadir , expframedir , expoframelist[i]) ;
        fits_open_file (&fptr , infile , READONLY , &status) ;
        printError (status , "***input File reading Fails***") ;
        fits_read_pix (fptr , TFLOAT , fpixel , xsize*ysize , NULL , frame_data_exp , NULL , &status) ;//Reading the Exposure Frame.
        printError (status , "***Error in reading pixels from input frame***") ;
        fits_read_key (fptr , TUSHORT , "FRAMENO" , &frameno , NULL , &status) ;
        printError (status , "***FRAMENO keyword not Found***") ;
        fits_read_key (fptr , TDOUBLE , "FRMTIME" , &frametime , NULL , &status) ;
        sprintf (outfile , "%s/%s/%s_t%.4f_f%d_exp_od.fits" , moduleoutdir , expframedir , nameprefix , frametime , frameno) ;
        ...
        for (int i = 0 ; i < xsize * ysize ; i++)
        {
            ...
            rown = rown + caldb_ydist_final[rown * xsize + coln] ;
            coln = coln + caldb_xdist_final[rown *  xsize + coln] ;
            frame_data_exp_temp[(int) (round (rown) *  xsize + coln)]=INVALID_PIX_VALUE;
            if (round (rown *  xsize + coln) < xsize * ysize)
            {
                frame_data_exp_temp[(int) (round (rown) *  xsize + coln)] = frame_data_exp[i] ;
            }
        }
        ...
        fits_write_pix (fout , TFLOAT , fpixel , xsize*ysize , frame_data_exp_temp , &status) ;// Writing the Corrected Exposure frames to theoutput Exposure frame
        printError (status , "***Error in writing the pixels to output signal file***",outfile) ;
        copyUserKeywords (fptr , fout) ;
        writeCommonKeywords (fout , modulename) ;
        fits_close_file (fout , &status) ;
...
int uvtOpticalDestortion_l2::opticalDestrotionPC ()
{
    ...
    printError (status , "Error in opening  the file") ;

    fits_create_file (&fevt_out , file_out , &status) ;
    printError (status , "Error in creating the output event file") ;
...
int uvtOpticalDestortion_l2::getHistory (vector<string> &vhistory)
{
    int cnt=0;
    char *user = getlogin () ;
...
//int uvtOpticalDestortion_l2::readDestortionFile() {
//    LOG(INFO) << endl << "Reading Distortion Correction file from caldb........" ;
//    fitsfile *fdist ;
//    int status = 0 ;
//    char errstr[500] ;
//
//    fits_open_file (&fdist , distrotionCorrfile , READONLY , &status) ;
...
//int uvtOpticalDestortion_l2::Applypadding(float *inputArray, float *outputArray) {
//
