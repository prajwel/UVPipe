...
bool compare_regAvg (struct Star star1 , struct Star star2) ;
...
int uvtRegAvg::uvtRegAvgProcess ()
{
  ...
  //  xsize=IMG_DIM_FI;
  //  ysize=IMG_DIM_FI;
   // char sigFileDir[FLEN_FILENAME];
  //  char expFileDir[FLEN_FILENAME];
  //  sprintf(sigFileDir,"%s/%s",inputdatadir,"SignalFrames");
  //  sprintf(expFileDir,"%s/%s",inputdatadir,"ExposureFrames");
//    vector<string> fileSig,fileExp;
//    status=getFiles (sigFileDir,fileSig);
//    status=getFiles (expFileDir,fileExp);
//    sigframelist = allocateMemory<char>(fileSig.size () , NAMESIZE) ;
//    expoframelist = allocateMemory<char>(fileExp.size () , NAMESIZE) ;
    //strcpy(sigframedir,"SignalFrames");
    //strcpy(expframedir,"ExposureFrames");
//      char name_temp[1000];
//       string tempstr=" ";
//       int cnt=0;
//    for(int i=0;i<fileSig.size ();i++)
//    {
//       // fileSig[i]="SignalFrames"+"/"+(string)basename(fileSig[i].c_str ());
//       
//     //   fileExp[i]="ExposureFrames"+"/"+(string)basename(fileExp[i].c_str ());
//        sprintf(name_temp,"%d",i+1);
//              tempstr="_f"+(string)name_temp+"_";
//          //    cout<<tempstr<< " "<<fileSig[i]<<endl;
//                   for(int j=0;j<fileSig.size ();j++)
//                   {
//                     if(strstr((char*)fileSig[j].c_str (),(char*)tempstr.c_str ())!=NULL)
//                        {
//        strcpy(sigframelist[cnt],basename((char*)fileSig[j].c_str ()));
//        strcpy(expoframelist[cnt],basename(((char*)fileExp[j].c_str ())));
//                         LOG(INFO)<<sigframelist[i];
//                         cnt++;
//                         break;
//                        }
//                       
...
//  
    ...
    sprintf (infofile_out , "%s/%s_rg.info" , moduleoutdir , nameprefix) ;
    ...
    printError (status , "Error in reading the file" , infofile_in) ;
    return (EXIT_SUCCESS) ;
}
...
int uvtRegAvg::reg_Avg ()
{
    ...
    int naxis = 2 ;
    long naxes[2] = {FINALFRAMESIZE_REGAVG , FINALFRAMESIZE_REGAVG} ;
...
//                framevector[i].print () ;
        ...
        else if (algo_flag == 0)
        {
            ...
            while (cnt < min_stars_match)
            {
                ...
                cnt = matchStars (cx_ref.size () , Cx.size () , div_fact , cx_ref.data () , cy_ref.data () , Cx.data () , Cy.data () , x_ref_arr , y_ref_arr , x_arr ,
           ...
           these are  the different techniques for finding shifts between two point of two frames.
             */
            status = findShiftsNtheta (cnt , x_ref_arr , y_ref_arr , x_arr , y_arr , temp_x_arr , temp_y_arr , x_dx , y_dy , theta_dt) ;
            if (status)
            {
           ...
           // theta_dt=0;
            ...
            for (int i = 0 ; i < xsize ; i ++)
            {
                ...
                for (int j = 0 ; j < ysize ; j ++)
                {
                     if(sig_array[j*xsize+i]!=INVALID_PIX_VALUE  && exp_array[j*ysize+i]!=INVALID_PIX_VALUE){
                    
                    int y_index = j - ysize / 2 ;
                    float new_index_x = round ((x_index) * ctheta - (y_index) * stheta + xsize / 2) - x_dx*mult_term ; //new index x
                    float new_index_y = round ((x_index) * stheta + (y_index) * ctheta + ysize / 2) - y_dy*mult_term ; //new index y

                    if (round (new_index_x) < xsize && round (new_index_x) > 0 && round (new_index_y) > 0 && round (new_index_y) < ysize)
                    {
                        cnt_loop ++ ;
        ...
        {
           if(sig_array_temp[p]!=INVALID_PIX_VALUE  && exp_array_temp[p]!=INVALID_PIX_VALUE){
    ...
    //for converting into 600*600  final frame
    ...
    //this algorithm converts the 9600*9600 frame to 600*600 frame.
    ...
    //writing signal frame
    sprintf (outfile , "%s/%s_sig_rg.fits" , moduleoutdir , nameprefix) ;
...
//    printError (status , "Error in DELETING  the FRM_TIME" , outfile) ;
    ...
    sprintf (outfile , "%s/%s_exp_rg.fits" , moduleoutdir , nameprefix) ;
    fits_create_file (&outfptr2 , outfile , &status) ;
    printError (status , "Error in creating the out file" , outfile) ;
    fits_create_img (outfptr2 , FLOAT_IMG , naxis , naxes , &status) ;
    printError (status , "Error in creating the image file" , outfile) ;
...
int uvtRegAvg::correlate (float *array1 , int h1 , int w1 , float *array2 , int h2 , int w2 , float xshift , float yshift , int bp_p1 , int bp_p2)
{
    ...
    for (int i = 0 ; i < h ; i ++)
    {
        for (int j = 0 ; j < w ; j ++)
        {
            im1[i * h + j] = image1[i][j] * pow (- 1.0 , i + j) ;
            im2[i * h + j] = image2[i][j] * pow (- 1.0 , i + j) ;
        }
    }
...
int uvtRegAvg::match (T *searchwindow , int h1 , int w1 , T *templatewindow , int h2 , int w2 , float *x , float *y)
{
    ...
    for (int i = 0 ; i < h ; i ++)
    {
        for (int j = 0 ; j < w ; j ++)
        {
            ...
            if (i < h1 && j < w1)
                im1[i * w + j] = searchwindow[i * w1 + j] * pow (- 1.0 , i + j) ;
            else
                im1[i * w + j] = 0 ;
            //copying template window to padded double array
            if (i < h2 && j < w2)
                im2[i * w + j] = templatewindow[i * w2 + j] * pow (- 1.0 , i + j) ;
            else
                im2[i * w + j] = 0 ;
        }
    }
...
label:
    ...
    // cout<<sd_mul_factor<<endl;exit(1);
    if (sd_mul_factor < 0)
    {
...
Star star1;
 ...
 sort (star_track.begin (),star_track.end (),compare_regAvg);
...
int uvtRegAvg::findShiftsNtheta (int totalelements , vector<float> &Xone , vector<float> &Yone , vector<float> &Xtwo , vector<float> &Ytwo ,
        vector<float> &DiffOfX , vector<float> &DiffOfY , double &Xdx , double &Ydy , double &Theta)
{
    if (option_LeastSquare == 1)
    {
        ...
        for (int t = 0 ; t < totalelements * 2 ; t = t + 2)
        {
            ...
            A (t , 0) = - 1.0 * (Yone[temp] - (xsize / mult_term) / 2) ;
            A (t , 1) = 1 ;
            A (t , 2) = 0 ;

            A (t + 1 , 0) = (Xone[temp] - (xsize / mult_term) / 2) ;
            A (t + 1 , 1) = 0 ;
            A (t + 1 , 2) = 1 ;
    ...
    else if (option_LeastSquare == 2)
    {
        ...
        {
            A (2 * aindex , 0) = Xone[aindex] - (xsize / mult_term) * 0.5 ;
            A (2 * aindex , 1) = - 1.0 * (Yone[aindex] - (xsize / mult_term) * 0.5) ;
            A (2 * aindex , 2) = 1.0 ;
            A (2 * aindex , 3) = 0.0 ;
            A (2 * aindex + 1 , 0) = (Yone[aindex] - (xsize / mult_term) * 0.5) ;
            A (2 * aindex + 1 , 1) = Xone[aindex] - (xsize / mult_term) * 0.5 ;
            A (2 * aindex + 1 , 2) = 0.0 ;
            A (2 * aindex + 1 , 3) = 1.0 ;
            B (2 * aindex , 0) = Xtwo[aindex] - (xsize / mult_term) * 0.5 ;
            B (2 * aindex + 1 , 0) = Ytwo[aindex] - (xsize / mult_term) * 0.5 ;
        }
        X.ApplyLeastSquare (A , B) ;
    ...
    else if (option_LeastSquare == 3)
    {
        ...
        for (int k = 0 ; k < totalelements ; k ++)
        {
            ...
            /* row 1 */
            double y_mod = Ytwo[k]-((xsize / mult_term) * 0.5) ;
            double x_mod = Xtwo[k]-((xsize / mult_term) * 0.5) ;
...
int uvtRegAvg::matchStars (int numrowsFirstfile , int numrowsSecfile , float divFact ,
