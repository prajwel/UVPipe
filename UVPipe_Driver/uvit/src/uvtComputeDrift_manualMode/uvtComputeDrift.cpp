...
{
    int status = 0 ;
    status = readParams (argc , argv , 4 , FNAME , "inputdatadir" , inputdatadir ,
...
//     }
 ...
 Yshift_arr[0] = 0 ;
  ...
  of1<<"==========================================================================================================================="<<endl;
    //loop For reading the number of frames in the input Directory 
  bool flag_matchedStar=FALSE;
  char NonMatchedstarList[FLEN_FILENAME];
  ofstream ofunmatched;
    for (int p = 0 ; p < nframes - 1 ; p ++) 
    { 
     ...
     //   minimum_noTargetStars=100;
        ...
        while(cnt <num_matchForExecution )
       {
        ...
        //removing the non-matched star and marked as a invalid stars
          sprintf (NonMatchedstarList , "%s/UNmatchstars_%d.txt" , moduleoutdir,p+1) ;
    ofunmatched.open (NonMatchedstarList , ios::out) ;
    ofunmatched<<"================================================="<<endl;
    ofunmatched<<"X location"<<setw(20)<<"Y location"<<setw(20)<<endl;
    ofunmatched<<"================================================="<<endl;
         for (int star_index_totalStars = 0; star_index_totalStars < numrows_secondFile; star_index_totalStars++) {
            flag_matchedStar = FALSE;
            for (int star_Index_match = 0; star_Index_match < int_vect.size(); star_Index_match++) {
                if (Xloc1[star_index_totalStars] == x_arr[star_Index_match] && Yloc1[star_index_totalStars] == y_arr[star_Index_match]) {//matched star
                    flag_matchedStar = TRUE;
                    break;

                }
            }
            if (flag_matchedStar == FALSE) {
               ofunmatched<<Xloc1[star_index_totalStars]<<setw(20)<<Yloc1[star_index_totalStars]<<endl;
                Xloc1[star_index_totalStars] = INVALID_PIX_VALUE;
                Yloc1[star_index_totalStars] = INVALID_PIX_VALUE;
               // int_vect[star_index_totalStars] = INVALID_PIX_VALUE;
...
//        }
        ...
        of1<<setw(20)<<p+1<<"-"<<p+2<<setw(20)<<minvalx<<setw(20)<<maxvalx<<setw(20)<<minvaly<<setw(20)<<maxvaly<<setw(20)<<maxvalx-minvalx<<setw(20)<<maxvaly-minvaly<<endl;
         ...
         {
//             Xloc1_temp[index]=Xloc1[index]+temp_cumm_x;
//             Yloc1_temp[index]=Yloc1[index]+temp_cumm_y;  
             if(Xloc1[index]!=INVALID_PIX_VALUE && Yloc1[index] !=INVALID_PIX_VALUE ){
             Xloc1_temp[index]=(Xloc1[index]-xsize/2)*cos(-temp_cumm_theta*M_PI/180)-(Yloc1[index]-ysize/2)*sin(-temp_cumm_theta*M_PI/180)-temp_cumm_x;
             Yloc1_temp[index]=(Xloc1[index]-xsize/2)*sin(-temp_cumm_theta*M_PI/180)+(Yloc1[index]-ysize/2)*cos(-temp_cumm_theta*M_PI/180)-temp_cumm_y;   
             Xloc1_temp[index]=Xloc1_temp[index]+xsize/2;
             Yloc1_temp[index]=Yloc1_temp[index]+ysize/2;
             }
             else{
            ...
            if (fittingflag) //decide fitting to be done or not
            {
               ...
               // cout<<X_shift_vect.size ()<<endl;
                ...
                for (int i = 0 ; i < X_shift_vect.size () ; i ++)
                {
                    ...
                    ofptr << time[i] <<X_shift_vect[i]<<setw(20)<<Y_shift_vect[i]<<setw(20)<<theta_vect[i]<< setw (20) << Xshift_arr[i] << setw (20) << Yshift_arr[i] << setw(20)<<theta_arr[i]<<endl ;
   ...
   // bool flag_theta_computation;
    ...
    { 
        ...
        vector<double> diff_x_cumm,diff_y_cumm;
          ...
          {
              LOG(INFO)<<" DX "<<Xtwo[t] - Xone[t]<<" DY "<<Ytwo[t] - Yone[t];
       ...
       maxEleY=*max_element(diff_y_cumm.begin(),diff_y_cumm.end());
        ...
        Theta=0.0f;
        LOG(INFO)<<"Total Elements "<<diff_x_cumm.size();
