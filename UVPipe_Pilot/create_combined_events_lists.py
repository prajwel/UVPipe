...
class correlate_events_lists:
    ...
    def get_shifts(self, bin_size=1, bg_bin_size=64, window_filter_episode=None):
        ...
        reference_bkg_image = self.get_image(
            self.reference_events_list, (0, 4801, bg_bin_size)
        )
        to_match_bkg_image = self.get_image(
            self.to_match_events_list, (0, 4801, bg_bin_size)
        )

        reference_bkg_image[reference_bkg_image <= 0] = np.nan
        to_match_bkg_image[to_match_bkg_image <= 0] = np.nan

        bg_reference = np.nanmedian(reference_bkg_image) / (bg_bin_size**2)
        bg_to_match = np.nanmedian(to_match_bkg_image) / (bg_bin_size**2)
...
def combine_event_lists(
    ...
    for path, matrix in zip(paths, matrices):
        ...
        hdu_base[0].header["COMBMETH"] = "DEFAULT_METHOD"
        hdu_base.close()
