...
class correlate_events_lists:
    def __init__(
        ...
        to_match_events_list,
        bright_source_x_coo,
        bright_source_y_coo,
    ):
        self.reference_events_list = reference_events_list
        self.to_match_events_list = to_match_events_list
        self.bright_source_x_coo = bright_source_x_coo
        self.bright_source_y_coo = bright_source_y_coo
    ...
    def get_columns(self, hdu):
        ...
        mask = np.logical_and(mask, bad_flag)
        source_mask = (
            (fx - self.bright_source_x_coo) ** 2 + (fy - self.bright_source_y_coo) ** 2
        ) <= 100**2
    ...
    def get_shifts(self, bin_size=1, bg_bin_size=64, window_filter_episode=None):
        ...
        # smoothed_reference_image = reference_image
        # smoothed_to_match_image = to_match_image
...
def combine_event_lists(
    ...
    for path, matrix in zip(paths, matrices):
        ...
        if i == 0:
            ...
            bright_sources = detect_sources_kdtree(
                hdu_base[1].data["Fx"], hdu_base[1].data["Fy"], 10
            )
            bright_source = bright_sources[bright_source_index]
            print(f"The bright source used for combining is at {bright_source}")
            ...
            init_correlation = correlate_events_lists(
                hdu_base, hdu_add, bright_source[0], bright_source[1]
            )
        ...
        hdu_base[0].header["COMBMETH"] = "ALT_METHOD2"
        hdu_base.close()
...
def make_combined_eventslists(channel, ref_episode=None, bright_source_index=0):
